...
if [ $? -eq 0 ]; then
    ...
    php -d mbstring.func_overload=0 phpunit.phar --bootstrap "$parentdir/vendor/autoload.php" "$parentdir/test/unit" && \
    # Run the test suite again with funky func_overload.
    php -d mbstring.func_overload=7 phpunit.phar --bootstrap "$parentdir/vendor/autoload.php" "$parentdir/test/unit"
    EXITCODE=$?
    # Cleanup
    if [ "$clean" -eq 1 ]; then
